...
const AddProduct = () => {
  ...
  const handleAddSubCategory = (subCategoryId) => {
    ...
    if (!subCategory) {
      console.error("Không tìm thấy danh mục phụ với ID:", subCategoryId);
      return;
    }

    const isAlreadySelected = data.subCategory.some(
              ...
              {data.sizes.map((s, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setData((prev) => {
                        const newSizes = prev.sizes.filter((_, i) => i !== idx);
                        return { ...prev, sizes: newSizes };
                      })
                    }
              ...
              {data.bases.map((b, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setData((prev) => {
                        const newBases = prev.bases.filter((_, i) => i !== idx);
                        return { ...prev, bases: newBases };
                      })
                    }
                    className="px-1 py-2 bg-blue-500  hover:bg-blue-600 rounded"
